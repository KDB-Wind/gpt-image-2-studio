...
test("release HTML falls back to session settings when localStorage is denied", async ({ page }) => {
  const pageErrors: string[] = [];
  page.on("pageerror", (error) => pageErrors.push(error.message));
  ...
  await page.getByTestId("settings-base-url").fill("https://session-only.example/v1");
  await page.getByTestId("settings-api-key").fill("session-only-key");
  await page.getByTestId("settings-text-model").fill("session-text-model");
  await page.getByTestId("settings-image-model").fill("session-image-model");
  await page.getByTestId("settings-save").click();

  await workspaceTabs.nth(0).click();
  await expect(workspaceTabs.nth(0)).toHaveAttribute("aria-selected", "true");
  await workspaceTabs.nth(3).click();
  await expect(page.getByTestId("settings-base-url")).toHaveValue("https://session-only.example/v1");
  await expect(page.getByTestId("settings-api-key")).toHaveValue("session-only-key");
  expect(pageErrors).toEqual([]);
});
