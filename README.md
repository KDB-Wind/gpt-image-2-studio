...
API key 的保存方式取决于运行形态：

- 网页模式默认只保留到当前浏览器会话结束；仅在可信的个人设备上启用“记住 API key”。
- 桌面模式关闭“记住 API key”时，密钥只保留在当前应用进程中；清空密钥或关闭记忆后保存，会删除当前供应商的系统凭据和本地回退副本。
- 桌面模式启用记忆后优先使用系统凭据库；系统凭据库不可用时会回退到应用配置文件，因此只应在可信设备上启用。
