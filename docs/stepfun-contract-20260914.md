...
1. **模型下线风险(最紧急)**:官方公告 `/images/edits` 接口与 `step-image-edit-2`、`step-2x-large` 将于 **2026-10-10 停止服务**(文生图/图生图/编辑三接口国内停服)。当前配置在 10-10 后不可用,需尽快决定迁移模型与时间表。
2. **是否为 t2i 增加独立模型字段**:结论**不需要**——step-image-edit-2 原生支持纯文生图(实测 200)。仅在下线换模型时再评估。
3. **是否增加 StepFun 内置 provider 预设**:建议**加**(baseUrl+模型下拉+尺寸白名单裁剪+64px 校验+默认 force-base64),偏差 1/4 会实际影响用户体验;不做 preset 也可用,但用户会踩 2K/4K 400 错误。
4. url 响应模式、file:// 直连实连未测(低风险,需要时可零成本补)。

## 7. 结论

StepFun 与本应用的 OpenAI 兼容契约**总体良好**:两条文本端点、两种生图路径、b64 返回、错误脱敏分类全部实测通过;不构成阻断。主要工作量为 preset 级适配(尺寸白名单/输入图校验)与 10-10 下线迁移决策。
