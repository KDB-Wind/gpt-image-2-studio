...
- [x] 真实目录碰撞不覆盖；同一浏览器应用实例内文件写入与历史写入使用原子事务。
- [x] 单图与批量任务均恰好请求一张图片；旧数量已规范化。
- [x] 桌面端和前端配置持久化完整覆盖图片响应模式及批量设置。
- [x] `v0.1.6` 发布与归档绑定；Git blob、链接、路径守卫完整；Release 严格校验与 Pages 非严格校验区分明确。
...
| P1 项目 | 状态 | 证据类别 |
| --- | --- | --- |
| 单任务重试并发与陈旧状态覆盖 | Fixed and unit-tested | 已审阅任务；451 测试与 retry mock E2E。 |
| AI 规划数量与 items 一致性 | Fixed and unit-tested；Real-provider verified | 已审阅任务；451 测试、mock E2E、真实服务规划 smoke。 |
| 面向用户的服务错误脱敏 | Fixed and unit-tested | 已审阅任务；451 测试与两层密钥扫描。 |
...
| 每任务恰好一张图片及旧数量规范化 | Fixed and unit-tested；Real-provider verified | 已审阅任务；451 测试、mock E2E、真实服务 E2E。 |
| 前端与桌面端完整配置持久化 | Fixed and unit-tested | 已审阅任务；451 测试与 mock E2E。 |
| `v0.1.6` 发布/归档守卫、最终一致性、可复现性和工作流测试 | Fixed and unit-tested | 已审阅任务；release/site 门禁、clean HEAD 可复现性及严格归档一致性。 |

### 2026-07-12 验证证据

- [x] `npm run test:run`：32 个文件、451 个测试通过。
- [x] `npm run build`：通过，47 个模块。
- [x] `npm run build:static`：通过，47 个模块。
- [x] `npm run site:check`：通过。
- [x] `npm run release:check`：21 个 readiness 测试，以及 readiness、clean HEAD 可复现性、严格归档一致性均通过。
...
- [x] 两层密钥扫描均通过。
- [x] `npx tsc --noEmit` 通过。
- [x] `cargo test`：25 通过；`cargo check` 通过。
- [x] `git diff --check` 通过。
...
- [x] `v0.1.6` 的源归档、分发 index 与 lite HTML：`A81D9D9C0E6E76C95BB66D16ED74F7283B0B3D90397060EF37B44AD2E5FEF129`
