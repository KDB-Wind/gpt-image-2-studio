...
- **Fixed and unit-tested：** `npm run test:run` 为 33 个文件、487 个测试通过；`npx tsc --noEmit` 通过；`cargo test` 为 28 通过；`cargo check` 通过。
...
- **构建与发布：** `npm run build` 通过（46 个模块），`npm run build:static` 通过（42 个模块）；`npm run site:check` 通过；`npm run release:check` 的 23 个 readiness 测试及 readiness、clean HEAD 可复现性、严格归档一致性均通过。
