...
### T10 推送前 README 与远端 CI 任务边界

- **请求结果与验收标准**:确认本地工作树干净，核对并补齐 README 对当前密钥保存语义的说明；提交后推送当前分支，并检查该 revision 的远端 CI 结果。
- **范围内文件/模块**:`README.md`、`README.en.md`、本任务记录，以及当前分支的 git push/GitHub Actions 只读检查。
- **明确排除与外部效果**:用户已明确授权推送当前分支并触发远端 CI；不合并、不切版、不创建 release、不部署 Pages、不修改分支保护。
- **范围扩展原因**:当前分支领先远端 21 个提交，但两份 README 均未随 B-8 调整，缺少 Web/桌面密钥存储与桌面 JSON fallback 披露；这是推送前用户指定的文档检查点。
- **未解决问题/阻塞**:远端 CI 结果需在推送后按当前 commit 核验；若失败，仅修复由本分支引起且在授权范围内的问题。
- **合并范围扩展**:首次推送后 PR #9 被 GitHub 标记为 `DIRTY`，没有生成 CI run；最新 `origin/main` 比共同基线多 27 个提交，包含 v0.1.8、依赖更新及与本分支重叠的可靠性/凭据修复。为落实“跑远端 CI”且不直接改写 main，允许在当前分支合并 `origin/main`、人工保留双方有效行为并运行完整门禁；不重写历史、不合并 PR。
- **发布门禁范围扩展**:合并后严格 `release:check` 的源码/归档可复现门禁按设计失败，因为 main 已冻结 v0.1.8，而本分支含新运行时代码。为使 PR CI 不覆盖 v0.1.8 且可生成一致的合并候选，当前分支仅准备 v0.1.9 版本号、release notes 与不可变静态归档；不创建 tag、Release 或 Pages 部署。
